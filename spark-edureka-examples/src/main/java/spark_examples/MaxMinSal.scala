...
object MaxMinSal {
  ...
  def main(args : Array[String]) {
     if(args.length != 2) 
      println("Usage : spark-submit --class MaxMinSalFinder <path to jar>/max-min-salary-finder_2.10-1.0.jar <file to read with full hdfs path> <file to save at hdfs directory>")
     else {
